    ...
    for (int row = 0; row < m; row++) {
        for (int col = mat_args->start_j; col < mat_args->end_j; col++) {
            ...
            for (int q = 0; q < num_block; q += 4) {
                ...
                __m256i w_lower = _mm256_and_si256(raw_w, lowMask);
                __m256i w_lower_next = _mm256_and_si256(raw_w_next, lowMask);
                __m256i w_upper = _mm256_and_si256(_mm256_srli_epi16(raw_w, 4), lowMask);
                __m256i w_upper_next = _mm256_and_si256(_mm256_srli_epi16(raw_w_next, 4), lowMask);
                ...
                w_0 = _mm256_sub_epi8(w_lower, zero_point);
                w_128 = _mm256_sub_epi8(w_upper, zero_point);
                w_0_next = _mm256_sub_epi8(w_lower_next, zero_point);
                w_128_next = _mm256_sub_epi8(w_upper_next, zero_point);
...
void MatmulOperator::mat_mul_all_techniques(struct matmul_params *params) {
    ...
    for (int i = 0; i < num_thread; ++i) {
        threads_args[i].start_j = (params->C.column / num_thread) * i;
        threads_args[i].end_j = (i == num_thread - 1) 
            ? params->C.column 
            : (params->C.column / num_thread) * (i + 1);
